...
const SCORE_2_PATTERNS: Record<RequirementDimension, RegExp> = {
  [RequirementDimension.SCALE]:
    /\b(\d[\d,]*[km]?\s*(rps|req(uests)?\/s(ec)?|rpm|tps|dau|mau|users?\/day|gb\/month|tb\/month)|\d+[km]?\s*concurrent|\d+[km]?\s*million)\b/i,
  ...
  [RequirementDimension.TEAM]:
    /\b(\d+\s*(engineer|dev|sre|devops|person|people)\s*(team)?|on[\s-]?call|kubernetes|k8s|ci\/cd\s*pipeline|zero[\s-]?downtime\s*deploy)\b/i,
  [RequirementDimension.BUDGET]:
    /\b(\$\d+|\d+\s*dollar|per\s*month|monthly\s*budget|cost[\s-]?optimi[sz]|reserved\s*instance|savings\s*plan|spot\s*instance)\b/i,
...
const SCORE_1_PATTERNS: Record<RequirementDimension, RegExp> = {
  ...
  [RequirementDimension.TEAM]:
    /\b(team|engineer|developer|devops|ops|small|large|startup|enterprise|managed\s*service)\b/i,
  [RequirementDimension.BUDGET]:
    /\b(budget|cost|cheap|expensive|affordable|free\s*tier|low\s*cost|price|spend)\b/i,
  [RequirementDimension.COMPLIANCE]:
    /\b(security|compliance|regulation|privacy|secure|audit|certif)\b/i,
};
